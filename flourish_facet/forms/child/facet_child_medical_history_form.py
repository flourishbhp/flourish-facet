...
from ...form_validators import FacetChildMedicalHistoryFormValidator
...
class FacetChildMedicalHistoryForm(SubjectModelFormMixin, forms.ModelForm):

    form_validator_cls = FacetChildMedicalHistoryFormValidator
        ...
        if not instance and previous_instance:
            for key in self.base_fields.keys():
                if key in ['child_chronic', ]:
                    ...
                    continue
                if key not in ['facet_visit', 'report_datetime', 'med_history_changed']:
    ...
    class Meta:
        model = FacetChildMedicalHistory
        fields = '__all__'
